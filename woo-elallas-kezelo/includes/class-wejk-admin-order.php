    ...
    public function display_returned_badge($item_id, $item, $product) {
        ...
        if (is_array($returned_items) && !empty($returned_items)) {
            ...
            if (in_array($product_id, $returned_items)) {
                echo '<div style="margin-top: 5px;"><span style="background: #dc3232; color: #fff; padding: 3px 6px; border-radius: 3px; font-size: 11px; font-weight: bold; text-transform: uppercase;">' . esc_html__('Lemondva / Elállás', 'woo-elallas-kezelo') . '</span></div>';
            }
        }
    }
}
