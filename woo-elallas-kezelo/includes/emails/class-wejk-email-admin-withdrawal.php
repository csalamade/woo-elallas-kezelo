...
class WEJK_Email_Admin_Withdrawal extends WC_Email {
    ...
    public function __construct() {
        ...
        $this->title          = __('Elállás / Lemondás értesítő (Admin)', 'woo-elallas-kezelo');
        $this->description    = __('Ezt az e-mailt az adminisztrátor kapja, miután egy vásárló elállást vagy lemondást kezdeményezett.', 'woo-elallas-kezelo');
    ...
    public function trigger($order_id, $is_pre_dispatch = false, $returned_items = array(), $target_status = '') {
        ...
        if ($this->is_enabled() && $this->get_recipient()) {
            ...
            if ($is_pre_dispatch && $subject === $this->get_default_subject()) {
                $subject = sprintf(__('Feladás előtti lemondás (Elállás) - Rendelés #%s', 'woo-elallas-kezelo'), $this->object->get_order_number());
            }
    ...
    public function get_default_subject() {
        return __('Új elállási kérelem - Rendelés #{order_number}', 'woo-elallas-kezelo');
    }

    public function get_default_heading() {
        return __('Elállási nyilatkozat érkezett', 'woo-elallas-kezelo');
    }
    ...
    public function get_content_html() {
        ...
        echo '<p>' . esc_html__('Kedves Admin!', 'woo-elallas-kezelo') . '</p>';
        
        if ($this->is_pre_dispatch) {
            echo '<p><strong>' . esc_html__('Egy vásárló még a feladás előtt lemondta a rendelését (elállt a vásárlástól). Kérjük, NE add fel a csomagot!', 'woo-elallas-kezelo') . '</strong></p>';
        } else {
            echo '<p>' . esc_html__('Egy vásárló élt az elállási jogával a weboldalon, miután a csomag feladásra került.', 'woo-elallas-kezelo') . '</p>';
        }
        
        echo '<ul>';
        echo '<li><strong>' . esc_html__('Rendelésszám:', 'woo-elallas-kezelo') . '</strong> ' . $this->object->get_order_number() . '</li>';
        ...
        echo '<thead><tr><th style="text-align:left; border: 1px solid #eee;">' . esc_html__('Termék azonosító', 'woo-elallas-kezelo') . '</th><th style="text-align:left; border: 1px solid #eee;">' . esc_html__('Név', 'woo-elallas-kezelo') . '</th><th style="text-align:left; border: 1px solid #eee;">' . esc_html__('Mennyiség', 'woo-elallas-kezelo') . '</th></tr></thead><tbody>';
        ...
        echo '<h3>' . esc_html__('Rendszer információ:', 'woo-elallas-kezelo') . '</h3>';
        if ($this->is_pre_dispatch) {
            echo '<p>' . sprintf(esc_html__('A rendelés státusza automatikusan a beállított \'%s\' állapotra módosult. Kérjük, intézkedj a visszatérítésről, ha a rendelés már fizetve lett.', 'woo-elallas-kezelo'), wc_get_order_status_name($this->target_status)) . '</p>';
        } else {
            echo '<p>' . esc_html__('A rendelés státusza automatikusan \'Felfüggesztve\' (on-hold) állapotra módosult. Kérjük, vedd fel a kapcsolatot a vásárlóval a további teendőkkel kapcsolatban.', 'woo-elallas-kezelo') . '</p>';
        }
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'recipient' => array(
                'title'       => __('Címzett(ek)', 'woo-elallas-kezelo'),
                'type'        => 'text',
                'description' => sprintf(__('Add meg, kinek menjen az értesítő (vesszővel elválasztva több cím is megadható). Alapértelmezett: <code>%s</code>.', 'woo-elallas-kezelo'), get_option('admin_email')),
