...
* **Reliable Validation:** In addition to client-side (JavaScript) checks, it also uses comprehensive server-side (PHP) validation.

== Installation ==

1. Upload the `woo-elallas-kezelo` folder to the `/wp-content/plugins/` directory, or install the plugin through the WordPress plugins screen directly.
...
No, the extension automatically integrates into the WooCommerce Order Tracking and My Account -> Orders pages.
